 ...
 * 仲裁顺序：用户显式选择的字典来源（最高） > 内置规则库 > pinyin-pro 语境 / 字典兜底。
 * 注意：PinyinService 不 import DictEngine（依赖方向 dict → pinyin 单向，避免环依赖）。
 */
 ...
 * 规则匹配策略：取所有命中的 context 中最长 pattern；无命中则回退 default。
 */
export function resolvePolyphone(char: string, context: string): string {
  ...
  for (const ctx of rule.contexts) {
    if (ctx.pattern && context.includes(ctx.pattern)) {
      if (!best || ctx.pattern.length > best.pattern.length) {
        best = ctx;
      }
    }
  }
  return best ? best.pinyin : rule.default || (rule.readings[0] ?? '');
}
...
export function annotate(
  ...
  try {
    ...
    const result: PinyinAnnotation[] = chars.map((ch, idx) => {
      ...
      // 2) 读音仲裁（优先级：用户字典 > canon语境 > 内置规则 > pinyin-pro）
      let readingSources: string[] | undefined;
      let readingVerified = false;
      const extReading = externalProvider?.resolve(logic, logicText);
      if (extReading) {
        // 用户显式选择，最高优先，已校验（源未知，不写 sources）
        py = extReading;
